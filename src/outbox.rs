...
#[instrument(skip_all)]
pub async fn process_next_task(pool: &SqlitePool, notion: &dyn NotionClient) -> Result<bool> {
    if let Some((id, _user_id, kind, ref_id, attempt)) = db::next_due_outbox(pool).await? {
        ...
        match res {
            ...
            Err(err) => {
                warn!(?err, id, kind, ref_id, attempt, "outbox task failed; backoff");
                db::backoff_outbox(pool, id, attempt).await?;
            }
        }
        return Ok(true);
    }
