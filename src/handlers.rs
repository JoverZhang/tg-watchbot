...
#[instrument(skip_all)]
pub async fn handle_update(bot: &Bot, pool: &SqlitePool, data_dir: &str, msg: &Message) -> Result<()> {
    ...
    let full_name = format!("{} {}", user.first_name, user.last_name.clone().unwrap_or_default());
        ...
        MessageKind::Common(common) => match &common.media_kind {
            MediaKind::Text(text) => {
                ...
                if text_content.trim() == "==BEGIN==" {
                    ...
                    let _rid = db::insert_resource(pool, user_id, batch_id, "text", text_content, message_id).await?;
                }
            }
            MediaKind::Photo(photo) => {
                // Pick the highest resolution photo
                if let Some(size) = photo.photo.last() {
                    let path = download_file(bot, data_dir, tg_user_id, message_id, size.file.id.as_ref()).await?;
                    let batch_id = db::current_open_batch_id(pool, user_id).await?;
                    let _rid = db::insert_resource(pool, user_id, batch_id, "photo", &path, message_id).await?;
                }
            }
            MediaKind::Video(video) => {
                let path = download_file(bot, data_dir, tg_user_id, message_id, video.video.file.id.as_ref()).await?;
                let batch_id = db::current_open_batch_id(pool, user_id).await?;
                let _rid = db::insert_resource(pool, user_id, batch_id, "video", &path, message_id).await?;
            }
            _ => {}
        },
        _ => {}
...
async fn download_file(bot: &Bot, data_dir: &str, tg_user_id: i64, msg_id: i32, file_id: &str) -> Result<String> {
...
mod tests {
    ...
    fn commit_title_parsing() {
        ...
        assert_eq!(parse_commit_title("==COMMIT== (Title)"), Some(Some("Title".to_string())));
